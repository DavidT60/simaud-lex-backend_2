...
export class AddScoringFieldsToHechosSimulacion1735067558000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!await queryRunner.hasColumn('hechos_simulacion', 'puntuacion_madre')) {
        ...
        if (!await queryRunner.hasColumn('hechos_simulacion', 'puntuacion_padre')) {
            await queryRunner.addColumn('hechos_simulacion', new TableColumn({
        ...
        if (!await queryRunner.hasColumn('hechos_simulacion', 'recomendacion_custodia')) {
            await queryRunner.addColumn('hechos_simulacion', new TableColumn({
