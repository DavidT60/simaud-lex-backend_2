...
export class NnaService {
  ...
  create(createNnaDto: CreateNnaDto): Promise<Nna> {
    const nna = this.nnaRepository.create(createNnaDto);
  ...
  async findOne(id: string): Promise<Nna> {
    const nna = await this.nnaRepository.findOne({ where: { id }, relations: ['procesos'] });
    if (!nna) {
