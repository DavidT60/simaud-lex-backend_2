...
export class ProcesoJudicialController {
  ...
  create(@Body() createDto: CreateProcesoJudicialDto, @Request() req) {
    return this.procesoService.create(createDto, req.user);
  }
  ...
  findAll(@Request() req) {
    return this.procesoService.findAll(req.user);
  }

  @Patch(":id/grade")
  @ApiOperation({ summary: "Calificar un proceso" })
  grade(
    @Param("id") id: string,
    ...
    return this.procesoService.gradeCase(id, calificacion, detalles, req.user);
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Generar simulación de sentencia basada en datos del proceso",
  })
  ...
  simularSentencia(
    ...
    const user = req?.user;
    return this.procesoService.generarSimulacionSentencia(
      id,
      simulateDto,
      user
    );
  ...
  @ApiResponse({
    status: 200,
    description: "Historial de simulaciones obtenido exitosamente",
  })
  ...
  @ApiOperation({
    summary: "Obtener casos similares de una simulación específica",
  })
